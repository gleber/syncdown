...
export async function parseMarkdownFile(
	absolutePath: string,
): Promise<{ frontmatter: any; bodyMd: string } | null> {
	try {
	...
	for (const file of markdownFiles) {
		...
		if (!record) {
			...
			createdSources.push({
				...
				lastModifiedLocal,
				snapshot: synthesizeSnapshotFromFrontmatter(
					integrationId,
					parsed.frontmatter,
					parsed.bodyMd,
				),
			...
			const localBodyHash = new Bun.CryptoHasher("sha256")
				.update(parsed.bodyMd)
				.digest("hex");
			const snapshotBodyHash = new Bun.CryptoHasher("sha256")
				.update(storedSnapshot.payload.bodyMd)
				.digest("hex");
			...
			if (localBodyHash !== snapshotBodyHash) {
				...
				if (
					parsed.frontmatter.title !== storedSnapshot.payload.title ||
					parsed.frontmatter.start !==
						storedSnapshot.payload.metadata.calendarStartAt ||
					parsed.frontmatter.end !==
						storedSnapshot.payload.metadata.calendarEndAt ||
					parsed.frontmatter.location !==
						storedSnapshot.payload.metadata.calendarLocation
				) {
					modified = true;
				}
			}
			...
			if (modified) {
				updatedSources.push({
					...
					lastModifiedLocal,
					snapshot: applyUpdatesToSnapshot(
...
function synthesizeSnapshotFromFrontmatter(
	integrationId: string,
	frontmatter: any,
	bodyMd: string,
): SourceSnapshot {
...
function applyUpdatesToSnapshot(
