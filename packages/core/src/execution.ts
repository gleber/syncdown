		...
		if (plugin.push && services.sink.analyzeLocalModifications) {
			...
			if (
				...
				const pushRequest = {
					...
					updatedSources: localChanges.updatedSources,
					deletedSourceIds: localChanges.deletedSourceIds,
				};
				const pushResult = await withRetries(
					`${integration.label} push`,
					io,
					() => plugin.push!(pushRequest),
					runtime,
				);
				request.throwIfCancelled();
				io.write(
